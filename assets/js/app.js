...
function initLoginHomeRedirect() {
    document.querySelectorAll('[data-auth-form="login"]').forEach((form) => {
        form.addEventListener("submit", (event) => {
            ...
            const message = form.querySelector("[data-auth-message]") || document.querySelector("[data-auth-message]");
            const loginField = form.elements.loginId || form.elements.login || form.elements.email || form.elements.phone;
            const passwordField = form.elements.password;
            const loginId = loginField?.value.trim() || "";
            ...
            if (!user || user.password !== password) {
                showMessage("Please check your login details and try again.", true);
                return;
            }
...
function initPersistentAuthNavigation() {
    ...
    nav.querySelector(".auth-logout-button").addEventListener("click", () => {
        clearActiveAccount();
        window.location.href = "index.html";
    });
}
